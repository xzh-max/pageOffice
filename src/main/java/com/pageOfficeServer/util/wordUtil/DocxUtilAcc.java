...
public class DocxUtilAcc {


    public static void main(String[] args){
        try {
            String path="E://药物疫苗采购合同模板(1).docx";
            XWPFDocument document = new XWPFDocument(POIXMLDocument.openPackage(path));
            List<String> params = search(document);
     ...
     */
    public static List<String> search(XWPFDocument document) {
        List<String> needPrams=new ArrayList<>();
        try {
            // 替换段落中的指定文字
...
//            com((List)itPara);
            ...
            while (itParaa.hasNext()) {       //遍历段落
                XWPFParagraph paragraph = (XWPFParagraph) itParaa.next();
                    ...
                    for(int i=0;i<run.size();i++)
                    {                       {
                            if(run.get(i).getText(run.get(i).getTextPosition()).trim().startsWith("${")&&
                                    run.get(i).getText(run.get(i).getTextPosition()).trim().endsWith("}")){
                                /**
...
//                                setTypeface(run.get(i),run.get(i));
                                needPrams.add(run.get(i).getText(run.get(i).getTextPosition()).trim());
                            }
                        }
                    }
            }
            ...
            for(int k=0;k<headerList.size();k++){
                ...
                for(XWPFParagraph xwpfParagraph:headerPara) {       //遍历段落
                    ...
                    for(int i=0;i<run.size();i++)
                    {
                        ...
                        {
                            if(run.get(i).getText(run.get(i).getTextPosition()).trim().startsWith("${")&&
                                    run.get(i).getText(run.get(i).getTextPosition()).trim().startsWith("}")){
                                /**
                                * 参数0表示生成的文字是要从哪一个地方开始放置,设置文字从位置0开始
                                * 就可以把原变量替换掉
                                */
